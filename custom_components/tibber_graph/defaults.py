...
To customize settings, edit config.py instead.

This file contains the base default values that are imported by config.py.
"""
...
# X-axis settings
SHOW_X_TICKS = True                         # If True, show x-axis ticks at manual label positions
START_AT_MIDNIGHT = False                   # True = show from midnight to midnight, False = current hour to last data point
X_AXIS_BOTTOM_MARGIN = 0.25                 # Space reserved for labels (0.22–0.28)
...
Y_AXIS_LABEL_PADDING = 11                   # Distance between tick and label (in points)
Y_AXIS_LABEL_VERTICAL = True                # If True, rotate Y-axis tick labels vertically
Y_AXIS_LABEL_VERTICAL_ANCHOR = True         # If True, anchor vertical labels at tick; False = center on tick
Y_AXIS_SIDE = "right"                       # "left" or "right" - which side to draw the Y axis on
Y_TICK_COUNT = 3                            # Set to an int to control number of Y-axis ticks (e.g., 5). None = automatic.
Y_TICK_USE_COLORS = False                   # True = color Y-axis ticks (min/avg/max); False = use default tick color

# Price labels
CURRENCY_OVERRIDE = "öre"                   # Override currency (e.g., "EUR", "NOK", "SEK"). None = use Tibber home currency
LABEL_CURRENT = True                        # True = show current price label
LABEL_CURRENT_AT_TOP = True                 # True = show current label at top-left corner; False = show on the graph
LABEL_CURRENT_AT_TOP_FONT_SIZE = 17         # Font size for top-left current label
...
LABEL_MAX_BELOW_POINT = True                # True = render max label below data point; False = render above
LABEL_MIN = True                            # True = show minimum price label on the graph
LABEL_MINMAX_SHOW_PRICE = False             # True = show price on min/max labels; False = show only time
LABEL_SHOW_CURRENCY = False                 # True = show currency on price labels (min/max/current).
LABEL_USE_COLORS = False                    # True = color min/max/avg labels (green/red/amber); False = use default label color
PRICE_DECIMALS = 2                          # Number of decimals to show for all price displays (e.g., 2 → 0.25)
USE_CENTS = True                            # True = display in cents
USE_HOURLY_PRICES = True                    # True = aggregate 15-min prices into hourly averages (4 quarters per hour)
...
# Refresh settings
AUTO_REFRESH_ENABLED = True                 # True = automatically refresh the chart on data interval boundaries (hourly or every 15 min)
